...
import { ChevronDown, ChevronUp, FilePlus2, FolderOpen, ListChecks } from 'lucide-react';
...
export default function VaultHealthCard({ vaultData, loading }: VaultHealthCardProps) {
  const { score, suggestions } = useVaultHealth(vaultData, loading);
  ...
  return (
    <div className="rounded-2xl border border-border bg-vault-panel shadow-vault overflow-hidden">
      <div className="p-5 flex flex-col sm:flex-row gap-5 sm:items-center">
        <div className="relative w-[120px] h-[120px] shrink-0 mx-auto sm:mx-0">
          <svg width="120" height="120" className="-rotate-90">
            <circle cx="60" cy="60" r={r} fill="none" stroke="color-mix(in srgb, var(--vault-c-elevated) 90%, transparent)" strokeWidth="10" />
            <circle cx="60" cy="60" r={r} fill="none" stroke="var(--vault-c-warm)" strokeWidth="10" strokeLinecap="round" strokeDasharray={`${c} ${c}`} strokeDashoffset={offset} className="transition-all duration-700" />
          </svg>
      ...
      <button type="button" onClick={() => setExpanded((e) => !e)} className="w-full flex items-center justify-center gap-2 py-2 text-xs font-700 text-vault-muted border-t border-border hover:bg-vault-elevated/40">
        {expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        {expanded ? 'Hide suggestions' : 'Show suggestions'}
      </button>
      {expanded && (
        <div className="px-5 pb-5 border-t border-border bg-vault-elevated/25">
          {loading ? (
            <p className="text-xs text-vault-muted pt-4">Loading…</p>
          ) : (
            <ul className="pt-4 space-y-3">
              {suggestions.map((s, i) => (
                <li key={`sug-${i}`} className="flex gap-3 rounded-xl border border-border bg-vault-panel px-3 py-3">
                  <ListChecks size={18} className="text-vault-warm shrink-0 mt-0.5" />
