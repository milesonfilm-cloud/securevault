...
function formatShortAdded(iso: string, locale: string): string {
  try {
...
export default function FamilyMemberWatchCard({
  ...
  const tw = useTranslations('familyWatchCard');
  ...
  const { shellShadow, shellShadowHover } = useMemo(() => {
    ...
    /** Coverflow (web + in-app WebView on iOS/Android): same shell as desktop carousel. */
    if (coverflowChild) {
      return {
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (frontTapOpensVault) {
                router.push(`/family-management?member=${encodeURIComponent(member.id)}`);
                return;
              }
              setFlipped(true);
            }}
            aria-expanded={flipped}
            aria-label={tw('viewDetailsAria', { name: member.name })}
              ...
              <div className={cn('flex min-h-0 w-full flex-1 flex-col', compact ? 'mt-2' : 'mt-4')}>
                ...
                >
                  <p
                    ...
                    style={{ fontFamily: WATCH_UI_INTER }}
                  >
                    {tw('memberSince', { date: formatShortAdded(member.createdAt, locale) })}
                  </p>
                ...
                */}
                  <div className="w-full min-w-0 -translate-y-0.5">
                    ...
                    >
                      {`${tw('vaultStamp')} · ${tm('categoryCount', { count: distinctCategories })}`}
                    </span>
                  </div>
                  ...
                  <p
                    ...
                    style={{ fontFamily: WATCH_UI_INTER }}
                  >
                    {frontTapOpensVault ? tw('tapOpenVault') : tw('tapForDetails')}
                  </p>
                  {frontTapOpensVault ? (
                    <button
                      type="button"
                ...
                >
                  ...
                  {compact ? (
                    ...
                    <div className="flex w-full min-w-0 -translate-y-0.5 justify-center">
                      ...
                      >
                        <span className="shrink-0">{tw('vaultWordmark')}</span>
