...
import { isMemberRelationshipValue } from '@/lib/memberRelationships';

function formatDOB(dob: string, locale: string): string {
  if (!dob) return '—';
  try {
    return new Date(dob).toLocaleDateString(locale, {
      day: '2-digit',
      month: 'short',
      year: 'numeric',
    });
...
export default function MemberCardBackContent({
  ...
  const tRel = useTranslations('memberForm.relationshipOptions');
  const relationshipLabel = (r: string) =>
    isMemberRelationshipValue(r) ? tRel(r) : r;

  const watch = surface === 'watch';
  const docCount = documents.length;
  const ageStr = (dob: string) => {
    const y = ageYears(dob);
    return y != null ? tm('ageYears', { years: y }) : '';
  };
  ...
  const vaultHref = `/family-management?member=${encodeURIComponent(member.id)}`;
    ...
    >
          ...
          <>
            ...
            >
              ...
              <div className="mt-1 flex flex-wrap items-center justify-center gap-x-1.5 gap-y-0.5">
                <span className="text-xs font-600 text-[#888]">
                  {relationshipLabel(member.relationship)}
                </span>
                {member.dob ? (
                  <>
                    <span className="text-[#555]">·</span>
                    <span className="text-xs text-[#888]">{ageStr(member.dob)}</span>
                  </>
                ) : null}
              </div>
            </div>
            ...
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
                  <span className="text-xs font-600 text-vault-muted">
                    {relationshipLabel(member.relationship)}
                  </span>
                  {member.dob ? (
                    <>
                      <span className="text-vault-faint">·</span>
                      <span className="text-xs text-vault-muted">{ageStr(member.dob)}</span>
                    </>
                  ) : null}
                </div>
              </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-2 text-[11px]">
          <div className="rounded-2xl border border-[color:var(--color-border)] bg-vault-elevated/35 px-2.5 py-2">
            <div className="mb-0.5 flex items-center gap-1 text-vault-faint">
              ...
              <span className="font-700 uppercase tracking-wider">{tm('profileAdded')}</span>
            </div>
          ...
          <div className="rounded-2xl border border-[color:var(--color-border)] bg-vault-elevated/35 px-2.5 py-2">
            <div className="mb-0.5 flex items-center gap-1 text-vault-faint">
              ...
              <span className="font-700 uppercase tracking-wider">{tm('lastActivity')}</span>
            </div>
            <p className="font-700 tabular-nums text-vault-text">
              {lastActivity ? formatShortDate(new Date(lastActivity).toISOString(), locale) : '—'}
            </p>
          </div>
        </div>
      ) : null}
      ...
      >
        ...
        >
          ...
          {tm('categoryCount', { count: distinctCategories })}
        </span>
      </div>
      ...
      {categoryBreakdown.length > 0 ? (
        <>
          <div
            ...
            aria-label={tm('categoriesWithDocsAria')}
          >
      ...
      {!demo ? (
        ...
        >
          <span>
            {compact && watch ? tm('viewInVaultCompact') : tm('viewInVaultFull')}
          </span>
