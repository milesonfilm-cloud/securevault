...
import DocumentPrivacyToggle from '@/components/permissions/DocumentPrivacyToggle';
...
interface DocumentListProps {
  ...
  /** Hide edit/delete/share (emergency read-only mode). */
  readOnly?: boolean;
  canEditDoc?: (doc: Document) => boolean;
  canDeleteDoc?: (doc: Document) => boolean;
  canShareLinkDoc?: (doc: Document) => boolean;
  canOpenShareWithFamily?: (doc: Document) => boolean;
  onOpenShareWithFamily?: (doc: Document) => void;
  onDocumentPrivacyChange?: (doc: Document, isPrivate: boolean) => void;
  canToggleDocumentPrivacy?: (doc: Document) => boolean;
}
...
export default function DocumentList({
  ...
  canDeleteDoc,
  canShareLinkDoc,
  canOpenShareWithFamily,
  onOpenShareWithFamily,
  onDocumentPrivacyChange,
  canToggleDocumentPrivacy,
}: DocumentListProps) {
  ...
  return (
    ...
    <div id="vault-document-list" className="space-y-2">
      {documents.map((doc) => {
        ...
        const allowEdit = canEditDoc ? canEditDoc(doc) : !readOnly;
        const allowDelete = canDeleteDoc ? canDeleteDoc(doc) : !readOnly;
        const allowShareLink = canShareLinkDoc ? canShareLinkDoc(doc) : !readOnly;
        const allowShareFamily =
          !!onOpenShareWithFamily && (canOpenShareWithFamily ? canOpenShareWithFamily(doc) : false);
        ...
        const actionButtons = (
          ...
          >
            ...
            {!readOnly && allowShareLink && (
              <button
                onClick={() => setShareDoc(doc)}
                className="p-1.5 rounded-[10px] text-vault-faint hover:text-vault-warm hover:bg-white/[0.05] transition-colors"
                title="Share document (time-limited link)"
              >
                <Share2 size={16} />
              </button>
            )}
            {!readOnly && allowShareFamily && (
              <button
                onClick={() => onOpenShareWithFamily?.(doc)}
                className="p-1.5 rounded-[10px] text-vault-faint hover:text-vault-warm hover:bg-white/[0.05] transition-colors"
          ...
          >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap min-w-0">
                  ...
                  {doc.isDigiLockerVerified && (
                    <span
                      className="shrink-0 inline-flex items-center gap-0.5 text-[9px] font-800 uppercase tracking-wider px-2 py-0.5 rounded-full bg-sky-500/20 text-sky-100 border border-sky-400/30"
                      title="DigiLocker verified import"
                    >
                      <BadgeCheck size={11} className="opacity-90" />
                      DigiLocker
                    </span>
                  )}
                </div>
            ...
            {isExpanded && (
              <div className="relative z-[1] px-5 pb-4 pt-1 animate-slide-up border-t border-border">
                ...
                {canToggleDocumentPrivacy?.(doc) && onDocumentPrivacyChange ? (
                  <DocumentPrivacyToggle
                    document={doc}
                    disabled={readOnly}
                    onChange={(next) => onDocumentPrivacyChange(doc, next)}
                  />
                ) : null}
    ...
    {!readOnly && shareDoc && (
      <ShareDocumentModal doc={shareDoc} isOpen onClose={() => setShareDoc(null)} />
    )}
    </>
  );
}
